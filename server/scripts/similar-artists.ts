...
// Usage: npm run similar-artists [-- --max-age=90d] [--artist-id=123] [--force]
//
...
// Support --max-age=90d as a single token
...
async function main() {
  console.log(`\n🎵 Last.fm Similar Artists Lookup`)
  ...
  for (const artist of artists) {
    ...
    if (result.skipped) {
      process.stdout.write(`⏭️  skipped (checked recently)\n`)
      totalSkipped++
    } else {
